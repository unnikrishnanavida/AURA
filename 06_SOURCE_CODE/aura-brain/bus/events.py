"""Event primitives for the bus subsystem."""
...
class BusEventBus:
    ...
    def subscribe(self, event_type: str, listener: Callable[[BusEvent], None]) -> None:
        """Register a listener for an event type."""
        self._listeners.setdefault(event_type, []).append(listener)

    def unsubscribe(self, event_type: str, listener: Callable[[BusEvent], None]) -> None:
        """Unregister a listener for an event type."""
        if event_type in self._listeners:
            self._listeners[event_type] = [l for l in self._listeners[event_type] if l is not listener]

    def publish(self, event: BusEvent) -> list[Any]:
